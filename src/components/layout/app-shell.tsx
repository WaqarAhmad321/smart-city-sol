...
import { Bell, BarChart2, MapPin, MessageSquare, Vote, AlertTriangle, Wrench, ListChecks, Settings, LifeBuoy, Home, Users } from "lucide-react";
...
const allNavItems: NavItem[] = [
  ...
  { title: "Field Assistance", href: "/field-assistance", icon: <Wrench />, officialOnly: true },
...
const CitySyncLogo = () => (
  <Link href="/dashboard" className="flex items-center gap-2 px-2 group-data-[collapsible=icon]:justify-center">
    ...
    <span className="font-bold text-lg group-data-[collapsible=icon]:hidden font-headline">CitySync</span>
  </Link>
);
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const navItems = allNavItems.filter(item => {
    ...
    if (item.officialOnly) return userRole === 'admin' || userRole === 'official';
    // Specific logic for citizen-only or shared items
    if (item.href === '/report-issue' || item.href === '/polling') return userRole === 'citizen' || userRole === 'admin'; // Admins can see citizen views
